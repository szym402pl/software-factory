...
description: Shared vocabulary for designing deep modules. Use when the user wants to design or improve a module's interface, find deepening opportunities, decide where a seam goes, make code more testable or context-legible, or when another skill needs the deep-module vocabulary.
---
...
- **Import from the defining module, not a re-exporter.** Import directly from the module that owns the symbol, not from a middleman that re-exports it (a barrel file, an `__init__.py` that re-exports the whole package, a `public-api.ts`). Re-exporters obscure the real dependency graph, make circular imports invisible until runtime, and force every consumer to load the entire package to use one function. Direct imports make dependencies explicit: you can read the top of a file and see exactly what it depends on.
- **Depth is a property of the interface, not the implementation.** A deep module can be internally composed of small, mockable, swappable parts — they aren't part of the interface. A module can have **internal seams** (private to its implementation, used by its own tests) as well as the **external seam** at its interface.
...
- **Exploring alternative interfaces** — see [DESIGN-IT-TWICE.md](DESIGN-IT-TWICE.md): spin up parallel sub-agents to design the interface several different ways, then compare on depth, locality, and seam placement.
